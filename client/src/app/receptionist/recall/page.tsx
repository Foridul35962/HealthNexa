...
import { recallSkippedPatient } from "@/store/slice/receptionistSlice";
...
export default function ScanPage() {
    ...
    if (result && isScanning) {
      ...
      if (text) {
        ...
        try {
          ...
          await dispatch(recallSkippedPatient({
  ...
  // --- 3. PREMIUM WHITE & BLUE RECALL SCANNER VIEW ---
  return (
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-6 z-20">
        <div className="text-center mb-8 max-w-xs">
          <div className="inline-flex items-center gap-1.5 text-xs text-blue-600 font-semibold mb-1.5 uppercase tracking-widest">
            <UserCheck size={12} /> Re-entry Gateway
          </div>
          <h1 className="text-2xl font-black tracking-tight text-slate-900">Recall Skipped Patient</h1>
          <p className="text-slate-400 text-[11px] mt-1 leading-relaxed">
            Scan the missed appointment pass token to re-inject the patient into the active chamber sequence.
          </p>
        </div>
          ...
          <AnimatePresence>
            {loading && (
              ...
              >
                ...
                <p className="font-bold text-[11px] uppercase tracking-widest text-slate-700">Revalidating Index...</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      >
        <div className="flex items-start gap-3 bg-white border border-slate-200 p-4 rounded-2xl max-w-xs w-full shadow-xs">
          ...
          <div className="space-y-0.5">
            <h4 className="text-[11px] font-bold text-slate-800 tracking-wide">Queue Synchronization</h4>
            <p className="text-[10px] text-slate-400 leading-relaxed">
              Successful scans will automatically dispatch a real-time signal to restore the candidate's priority in the live schedule matrix.
            </p>
          </div>
        </div>
      </motion.div>
